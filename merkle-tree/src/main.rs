...
use plonk_core::prelude::{verify_proof, Circuit, StandardComposer, VerifierData};
...
fn main() {
    let generate_setup = false;
    let mut rng = test_rng();

    if generate_setup {
        let size = 1 << (HEIGHT + 9);
        let pp = KZG10::<Bls12_381>::setup(size, None, &mut rng).unwrap();
        io::store_params(pp, "merkle-tree/setup/setup2");
        return;
    }
    ...
    // ==============================
    {
        ...
        let size = 1 << (HEIGHT + 9);
        // let pp = KZG10::<Bls12_381>::setup(size, None, &mut rng).unwrap();
        let pp: ark_poly_commit::kzg10::UniversalParams<Bls12<ark_bls12_381::Parameters>> = io::load_params("merkle-tree/setup/setup2");
        ...
        let (proof, pi) = {
            ...
            real_circuit.gen_proof::<KZG10<Bls12_381>>(&pp, pk, b"Merkle tree").unwrap()
        };
